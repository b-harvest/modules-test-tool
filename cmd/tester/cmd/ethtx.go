...
func NewEvmTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "evmtx [round] [tx-num]",
		Short: "Broadcast erc20 tx",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// get ethereum address using public key
			...
			log.Info().Msgf("wait 5 sec erc20 contract to be deployed: %s", signedTx.Hash().Hex())
			...
			for i := 0; i < round; i += 1 {
				nonce, _ := ethCli.NonceAt(context.Background(), ethAddr, nil)
